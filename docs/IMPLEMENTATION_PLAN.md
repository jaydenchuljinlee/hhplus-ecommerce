...
- [x] `application.yml`에서 하드코딩된 secretKey를 환경변수 참조로 변경
  ```yaml
  jwt:
    secretKey: ${JWT_SECRET_KEY:local-dev-only-not-for-production-change-me}
  ```
- [x] `.env.example` 파일 생성 (환경변수 템플릿)
- [x] Docker Compose에 환경변수 전달 설정 추가
...
- [x] `build.gradle.kts`에 `spring-boot-starter-validation` 의존성 추가
- [x] Request DTO에 Bean Validation 어노테이션 추가
  - `OrderCreationRequest`: `@field:Min(1)` on userId, `@field:NotEmpty @field:Valid` on details
  - `PaymentRequest`: `@field:Min(1)` on orderId, userId
  - `BalanceChargeRequest`: `@field:Min(1)` on userId, amount (init block 제거)
  - `BalanceViewRequest`: `@field:Min(1)` on userId
  - `CartCreationRequest`: `@field:Min(1)` on userId, productId, quantity
  - `CartDeletionRequest`: `@field:Min(1)` on cartId
  - `CartListQueryRequest`: `@field:Min(1)` on userId
- [x] Controller 메서드 파라미터에 `@Valid` 추가 (IOrderController, IPaymentController, IBalanceController, ICartController)
- [x] `ApiControllerAdvice`에 `MethodArgumentNotValidException` 핸들러 추가 → 400 Bad Request
...
- [x] `ApiControllerAdvice`에 다음 핸들러 추가:
  - `MethodArgumentNotValidException` → 400 (Step 5-3에서 완료)
  - `HttpMessageNotReadableException` → 400 (`handleHttpMessageNotReadable` override)
  - `MethodArgumentTypeMismatchException` → 400 (`@ExceptionHandler`)
  - `NoHandlerFoundException` → 404 (`handleNoHandlerFoundException` override)
- [x] 에러 응답 포맷 일관성 확보 (모두 `CustomErrorResponse` 사용)
...
- [x] 매핑 개선 완료:
  ...
  | FacadeException | 422 | **409 Conflict** |
  | RepositoryException | 500 | 500 (유지) |
  | InsufficientBalanceException | 500 (상위) | **409 Conflict** |
  | OutOfStockException | 500 (상위) | **409 Conflict** |
  | BalanceLimitExceededException | 500 (상위) | **409 Conflict** |
  | DuplicatedProductException | 500 (상위) | **409 Conflict** |
  | InvalidOrderStatusException | 500 (상위) | **409 Conflict** |
  | NotFoundException 계열 | 500 (상위) | **404 Not Found** |
- [x] `common/exception/NotFoundException` 추상 클래스 도입 (`RepositoryException` 상속)
- [x] `BalanceNotFoundException`, `OrderNotFoundException`, `OrderDetailNotFoundException`, `UserNotFoundException`, `ProductNotFoundException`, `CartNotFoundException` 모두 `NotFoundException` 상속으로 변경
- [ ] 변경 후 기존 테스트 업데이트
...
- [x] `BalanceService` 단위 테스트 (`domain/balance/BalanceServiceTest`)
  - 잔액 조회 / 충전 성공 / 충전 한도 초과 / 사용 성공 / 잔액 부족 / 최소 사용 금액 미만
- [x] `OrderService` 단위 테스트 (`domain/order/OrderServiceTest`)
  - 주문 생성 / 주문 조회 / confirmStock / failStock / orderComplete / deleteOrderDetail
  - 잘못된 상태 전이 시 InvalidOrderStatusException 검증
- [x] `PaymentService` 단위 테스트 (`domain/payment/PaymentServiceTest`)
  - 결제 성공 / PAID 상태 반환 / 저장 엔티티 검증 / 이벤트 발행 확인
- [ ] `ProductService` 단위 테스트 (기존 통합 테스트로 대체)

**대상 파일**: `src/test/kotlin/com/hhplus/ecommerce/domain/` 하위 각 도메인 테스트

---

### Step 7-2. 엣지 케이스 & 경계값 테스트 ✅ (Step 7-1에 통합)

- [x] 충전 후 잔액이 정확히 MAX(100,000,000)일 때 성공
- [x] 충전 후 잔액이 MAX 초과 시 BalanceLimitExceededException
- [x] 사용 후 잔액이 정확히 MIN(100)일 때 성공
- [x] 사용 금액이 MIN 미만(99원)일 때 InsufficientBalanceException
- [x] 사용 후 잔액이 MIN 미만이면 InsufficientBalanceException
- [x] 잘못된 주문 상태 전이 → InvalidOrderStatusException
- [ ] 재고 0개 상품 주문 (기존 ProductDetailEntityTest 커버)
- [ ] 존재하지 않는 사용자/주문/상품 ID로 요청 (통합 테스트 영역)
...
- [x] `application.yml` (공통): `ddl-auto: none` (기본값 안전하게)
- [x] `application-local.yml`: `ddl-auto: create` (로컬 개발용)
- [x] `application-dev.yml`: `ddl-auto: update` + kafka 토픽 버그 수정
- [x] `application-test.yml`: `ddl-auto: create` (테스트 격리)
- [x] `application-prod.yml` 신규 생성: `ddl-auto: validate`, `sql.init.mode: never`, 모든 값 환경변수 참조
...
- [x] `RedisLockAspect`에서 Redis 연결 실패 시 Fallback 구현:
  - `RedisException` / `DataAccessResourceFailureException` catch
  - WARN 로그 출력 후 `executeWithoutLock()` — 분산락 없이 트랜잭션만 보장하여 실행
  - 주석으로 "데이터 정합성 위험" 및 "모니터링 연동 권장" 명시
- [ ] 서킷 브레이커 패턴 적용 (Resilience4j) — 장기 과제
- [ ] Redis 장애 시나리오 테스트 (TestContainers에서 Redis 중단 시뮬레이션) — 장기 과제
...
- [x] MySQL healthcheck: `mysqladmin ping` (interval: 10s, retries: 5, start_period: 30s)
- [x] Redis healthcheck: `redis-cli ping`
- [x] MongoDB healthcheck: `mongosh --eval "db.adminCommand('ping')"`
- [x] Zookeeper healthcheck: `echo ruok | nc 2181`
- [x] Kafka healthcheck: `kafka-broker-api-versions --bootstrap-server localhost:9092`
- [x] spring-app `depends_on` → `condition: service_healthy` (redis, mysql, mongodb, kafka)
- [x] kafka-ui `depends_on` kafka → `condition: service_healthy`
...
- [x] `GET /balance` → `@RequestParam @Min(1) userId: Long` 으로 변경, `@Validated` 추가
- [x] `GET /cart/list` → `@RequestBody` → `@RequestParam @Min(1) userId: Long` 으로 변경
- [x] `CustomApiResponse.created()` 팩토리 메서드 추가 (code=201, status=CREATED)
- [x] 응답 HTTP 상태 코드 정리:
  - POST 생성 성공: `201 Created` (addCart, prepareOrder, payment)
  - DELETE 성공: `204 No Content` (deleteCart → ResponseEntity<Void>)
  - 조회 성공: `200 OK` (유지)
  - PATCH 충전: `200 OK` (유지)
...
- [x] `RedisLockStrategy` enum에서 미구현 전략(SIMPLE, SPIN, FAIR) 제거 → `PUB_SUB`만 유지
- [x] `RedisLockAspect`의 `when` 분기 제거 → `pubSubLockSupporter` 직접 호출로 단순화
- [x] `@RedisLock`의 `strategy` 파라미터 유지 (추후 전략 추가 시 확장 지점)
- [ ] 각 전략별 IRedisLockSupporter 구현체 추가 — 장기 과제
...
| Phase | 설명 | 상태 |
|-------|------|------|
| **Phase 1** | 즉시 수정 (버그 & 코드 품질) | ⚠️ 일부 완료 (Step 1-4 IDE Refactoring 제외) |
| **Phase 2** | 주문~결제 비즈니스 흐름 재설계 | ✅ 완료 (Step 2-5) |
| **Phase 3** | 이벤트 & 메시징 안정화 | ✅ 완료 (Step 3-1 ~ 3-4) |
| **Phase 4** | 아키텍처 레이어 분리 | ✅ 완료 (Step 4-1) |
| **Phase 5** | 보안 강화 | ✅ 완료 (Step 5-1 ~ 5-3) |
| **Phase 6** | 예외 처리 보강 | ✅ 완료 (Step 6-1 ~ 6-2) |
| **Phase 7** | 테스트 보강 | ✅ 완료 (Step 7-1 ~ 7-2) |
| **Phase 8** | 인프라 & 운영 안정화 | ✅ 완료 (Step 8-1 ~ 8-3) |
| **Phase 9** | API 설계 개선 | ✅ 완료 (Step 9-1 ~ 9-2) |
